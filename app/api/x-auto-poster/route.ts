...
export async function POST(request: NextRequest) {
  try {
    console.log('Starting X immediate post...')
    ...
    if (!text || text.trim() === '') {
      return NextResponse.json({
        success: false,
        error: 'Text content is required for X post'
      }, { status: 400 })
    }
    
    // Run the X immediate poster
    const pythonPath = path.join(process.cwd(), 'x_auto_poster', 'x_api.py')
    ...
    return new Promise<Response>((resolve) => {
      ...
      const timeout = setTimeout(() => {
        ...
        resolve(NextResponse.json({
          success: false,
          error: 'X post timed out after 30 seconds'
        }, { status: 500 }))
      }, 30000)
      ...
      child.on('close', (code) => {
        ...
        console.log('X poster process finished with code:', code)
        ...
        if (code === 0) {
          ...
          resolve(NextResponse.json({
            success: false,
            error: `X post failed with code ${code}`,
            stderr: stderr,
            stdout: stdout
          }, { status: 500 }))
        }
      ...
      child.on('error', (error) => {
        clearTimeout(timeout)
        console.error('X poster process error:', error)
        resolve(NextResponse.json({
          success: false,
          error: `Failed to start X poster: ${error.message}`
        }, { status: 500 }))
      })
    })
