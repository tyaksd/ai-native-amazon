...
function BrandPageWithoutClerk({ 
  params, 
  hideHeader = false,
  user: providedUser,
  ...
  // Use provided favorites hooks or fallback to useFavoritesFallback
  const fallbackFavorites = useFavoritesFallback()
  ...
  if (!brand) {
    return (
      <div className="px-6 py-10 bg-[#FAFAF7]">
...
function BrandPageInner({ params }: PageProps) {
  ...
  // Reuse BrandPageWithoutClerk for layout - only authentication logic differs
    return (
    <BrandPageWithoutClerk 
      params={params}
      user={user}
