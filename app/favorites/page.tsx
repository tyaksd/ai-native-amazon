  ...
  useEffect(() => {
    const loadFavorites = async () => {
      try {
        ...
        if (isLoggedIn && clerkId) {
          // Use Clerk ID if logged in
          userId = clerkId
        } else {
          // Fallback to session_id from localStorage for non-logged-in users
        ...
        const { data: favorites, error: favoritesError } = isLoggedIn
          ? await supabase
              .from('favorites')
              .select('product_id')
              .eq('clerk_id', userId)
        ...
        const products = await Promise.all(
          favorites.map(async (favorite: { product_id: string }) => {
            try {
  ...
  if (favoriteProducts.length === 0) {
    return (
      <div className="px-6 py-10 text-center bg-[#FAFAF7] min-h-screen">
        <div className="text-gray-600 text-lg mb-4">No favorite products yet</div>
...
function FavoritesPageInner() {
  ...
  // Reuse FavoritesPageFallback for layout - only authentication logic differs
  return (
