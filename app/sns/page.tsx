...
export default function SNSPage() {
  ...
  const renderContentWithLinks = (content: string, platform?: string) => {
    ...
    const linkPattern = platform === 'X' ? /(godship\.io\/brands)/g : /(godship\.io)/g
    const linkUrl = platform === 'X' ? 'https://godship.io/brands' : `https://godship.io/${postData.selectedBrand.id}`
    
    const parts = content.split(linkPattern)
    return (
      <span className="whitespace-pre-wrap">
        {parts.map((part, index) => {
          if (part === 'godship.io' || part === 'godship.io/brands') {
            return (
              <a
                key={index}
                href={linkUrl}
                ...
                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  {postData.selectedBrand.background_image && (
                    <div className="mt-3">
                      ...
                      />
                      <div className="hidden text-sm text-gray-500 mt-2 p-2 bg-gray-100 rounded">
