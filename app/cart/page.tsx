...
function CartFallback() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Load cart from database using session_id
      let items: CartItem[] = [];
      const sessionId = typeof window !== 'undefined' ? localStorage.getItem('session_id') : null;
      const dbItems = await loadCartFromDB(null); // Pass null for clerkId, use session_id
      items = dbItems;
      
      // If no items from database, try localStorage as fallback
      if (items.length === 0) {
  ...
  const removeFromCart = async (productId: string, size?: string | null, color?: string | null) => {
    ...
    // Update database using session_id
    await removeCartItemFromDB(null, productId, size, color);
  ...
  const updateQuantity = async (productId: string, newQuantity: number, size?: string | null, color?: string | null) => {
    ...
    if (item) {
      await saveCartItemToDB(null, item);
    }
